...
function characterSelectInit()
{
	...
	gameEngine.stage.on( "click", function ( evt ) { if ( evt.stageX < gameEngine.CANVASWIDTH / 2 ) characterMode = "jamie"; else characterMode = "halladay"; gameEngine.mode = "level1"; if ( titleMusic.playState == createjs.Sound.PLAY_SUCCEEDED ) titleMusic.stop(); } );
	if ( titleMusic.playState != createjs.Sound.PLAY_SUCCEEDED ) titleMusic.play( { loop: -1 } );
}
...
var mainGameQueue, jamieChara, halladayChara, pixel, healthBar, powerStar, easterEgg, gameMusic;
...
var Level3Manifest =
	[
		...
		{ src: "images/level3/level3Enemy.png", id: "level3Boss" },
		{ src: "images/level3/level3BackGround.png", id: "level3BackGround" },
		{ src: "images/level3/level3Shelf.png", id: "level3Shelf" },
	];
var Level3Queue, level3Frame, level3Enemy, level3Boss, level3BackGround, level3Shelf;
...
var Level4Queue, level4Frame, level4Enemy, level4Boss, level4BackGround, level4Chair, level4DoorRight, level4Lights, level4Table;

var Level5Manifest =
	[
		{ src: "images/level5/level5Frame.png", id: "level5Frame" },
		{ src: "images/level5/level5Enemy.png", id: "level5Enemy" },
		{ src: "images/level5/level5Enemy.png", id: "level5Boss" },
...
var Level5Queue, level5Frame, level5Enemy, level5Boss, level5BackGround, level5WoodPile, level5Building;
...
(
{
	...
	frames:
		[[0, 0, 69, 93, 0, 23.9, 20.1], [69, 0, 92, 107, 0, 38.9, 19.1], [161, 0, 76, 100, 0, 27.9, 22.1]],
	animations:
		{
			Neutral: [0, 0],
			Run:
	...
	level1Enemy.scaleX = 0.75;
	level1Enemy.scaleY = 0.75;
	level1Enemy.regX = level1Enemy.getTransformedBounds().width / 2;
	level1Enemy.regY = level1Enemy.getTransformedBounds().height;
	...
	level1EnemyIcon.regX = level1EnemyIcon.getTransformedBounds().width;
	level1EnemyIcon.regY = level1EnemyIcon.getTransformedBounds().height;
...
function level3Loaded()
{
	...
	level3Enemy.scaleX = 0.75;
	level3Enemy.scaleY = 0.75;
...
(
{
	...
	level5Enemy.scaleX = 0.75;
	level5Enemy.scaleY = 0.75;
...
function level1Init()
{
	...
	boss = new moveableAttacker( new moveableObject( level1Boss.clone(), new vec2( MAXDISTANCE, 0 ), 10 ), new shortRangeAttack( 0, -level1Boss.getTransformedBounds().height / 8, level1Boss.getTransformedBounds().width, 10 ), 50 );
	boss.icon = level1BossIcon.clone();

	boss.icon.x = gameEngine.CANVASWIDTH;
	boss.icon.y = gameEngine.CANVASHEIGHT - healthBar.getTransformedBounds().height;
	...
	for ( var i = 0; i < 10; i++ )
	{
		enemies.push( new moveableAttacker( new moveableObject( level1Enemy.clone(), new vec2( gameEngine.CANVASWIDTH + ( ( MAXDISTANCE - ( gameEngine.CANVASWIDTH * 2 ) ) * Math.random() ), gameEngine.CANVASHEIGHT * Math.random() ), Math.random() * 10 ), new shortRangeAttack( level1Enemy.getTransformedBounds().width / 4, -level1Enemy.getTransformedBounds().height / 4, 100, 20 ), 3 ) );
		...
		enemies[i].icon.y = gameEngine.CANVASHEIGHT - healthBar.getTransformedBounds().height;
...
function level3Init()
{
	...
	backDrops.push( new moveableBackdrop( level3Shelf, 1, new vec2( Math.random() * 20, 320 ), new vec2( 0, 0 ), new vec2( level3Shelf.getTransformedBounds().width + 500, 0 ), false, true ) );
	for ( var i = 0; i < backDrops.length; i++ )
	{
		for ( var j = 0; j < backDrops[i].array.length; j++ )
		{
	...
	boss = new moveableAttacker( new moveableObject( level3Boss.clone(), new vec2( 9000, 0 ), 10 ), new shortRangeAttack( 0, -level3Boss.getTransformedBounds().height / 8, level3Boss.getTransformedBounds().width, 10 ), 50 );
	boss.attacker.characterSprite = boss.moveable.sprite;
	boss.attacker.debugSprite = pixel.clone();
	boss.moveable.sprite.on( "animationend", function ( evt ) { if ( evt.name == "Die" ) evt.target.visible = false; } );
	spriteArray.push( boss.moveable );
	...
	for ( var i = 0; i < 100; i++ )
	{
		enemies.push( new moveableAttacker( new moveableObject( level3Enemy.clone(), new vec2( gameEngine.CANVASWIDTH + ( ( 10000 - gameEngine.CANVASWIDTH ) * Math.random() ), gameEngine.CANVASHEIGHT * Math.random() ), Math.random() * 10 ), new shortRangeAttack( level3Enemy.getTransformedBounds().width / 4, -level3Enemy.getTransformedBounds().height / 4, 100, 20 ), 3 ) );
		enemies[i].attacker.characterSprite = enemies[i].moveable.sprite;
		enemies[i].attacker.debugSprite = pixel.clone();
		enemies[i].attacker.debugSprite.visible = true;
		enemies[i].moveable.sprite.on( "animationend",
	...
	stageBounds = new cage( 333, gameEngine.CANVASHEIGHT, 0, 10000 );
	cameraBounds = new cage( 0, gameEngine.CANVASHEIGHT, 0, 10000 );
	camera = new vec2( 0, 0 );
...
function level4Init()
{
	...
	stageBounds = new cage( 400, gameEngine.CANVASHEIGHT, 0, 10000 );
	cameraBounds = new cage( 0, gameEngine.CANVASHEIGHT, 0, 10000 );
	camera = new vec2( 0, 0 );
...
function level5Init()
{
	...
	boss = new moveableAttacker( new moveableObject( level5Boss.clone(), new vec2( 9000, 0 ), 10 ), new shortRangeAttack( 0, -level5Boss.getTransformedBounds().height / 8, level5Boss.getTransformedBounds().width, 10 ), 50 );
	boss.attacker.characterSprite = boss.moveable.sprite;
	boss.attacker.debugSprite = pixel.clone();
	boss.moveable.sprite.on( "animationend", function ( evt ) { if ( evt.name == "Die" ) evt.target.visible = false; } );
	spriteArray.push( boss.moveable );
	...
	for ( var i = 0; i < 100; i++ )
	{
		enemies.push( new moveableAttacker( new moveableObject( level5Enemy.clone(), new vec2( gameEngine.CANVASWIDTH + ( ( 10000 - gameEngine.CANVASWIDTH ) * Math.random() ), gameEngine.CANVASHEIGHT * Math.random() ), Math.random() * 10 ), new shortRangeAttack( level5Enemy.getTransformedBounds().width / 4, -level5Enemy.getTransformedBounds().height / 4, 100, 20 ), 3 ) );
		enemies[i].attacker.characterSprite = enemies[i].moveable.sprite;
		enemies[i].attacker.debugSprite = pixel.clone();
		enemies[i].attacker.debugSprite.visible = true;
		enemies[i].moveable.sprite.on( "animationend",
